...
try (PreparedStatement stmtProduct = connection.prepareStatement(sqlProduct, Statement.RETURN_GENERATED_KEYS);
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    //SE DEVUEL EL ID STRING DE L ENTRADA
                    flower.setId(id);
                }
            }
        } catch (SQLException e) {
    ...
    public List <Flower> findAll() {
        ...
        try (Statement stmt = connection.createStatement();
             //faltaba espacio despues de color
    ...
    public boolean exists(Flower flower) throws SQLException {
        String sql = "SELECT COUNT(*) FROM product p JOIN flower f ON p.id_product=f.id_product WHERE p.name = ? AND f.color = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
...
public class MysqlFlowerDao implements FlowerDao<Integer> {
    ...
    public void create(Flower<Integer> flower) throws SQLException {
        ...
        try (PreparedStatement stmtProduct = connection.prepareStatement(sqlProduct, Statement.RETURN_GENERATED_KEYS);
            ...
            stmtProduct.setInt(2, flower.getStock());
            stmtProduct.setDouble(3, flower.getPrice());
            stmtProduct.setString(4, "FLOWER");
            int affectedRows = stmtProduct.executeUpdate();
            if (affectedRows == 0) {
            ...
            affectedRows = stmtFlower.executeUpdate();
            if (affectedRows == 0) {
                throw new SQLException("Error al introducir el elemento de flor en la base de datos");
            }
    ...
    public void updateStock(Integer id, int stockDiff) throws Exception {
        ...
        if (flower == null) {
            throw new Exception("La ID introducida no corresponde a ningún elemento");
        }
        int stock = flower.getStock();
        int newStock = stock + stockDiff;
        String sql = "UPDATE product SET stock = ? WHERE id_product = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            int affectedRows = stmt.executeUpdate();
            if (affectedRows == 0) {
                throw new Exception("No se ha producido ninguna modificación");
            }
        } catch (SQLException e) {
            logger.log(Level.SEVERE, "Error al modificar el stock del producto en la base de datos", e);
        }
    ...
    public void deleteById(Integer id) throws Exception {
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            int affectedRows = stmt.executeUpdate();
            if (affectedRows == 0) {
                throw new Exception("No se ha producido ningún borrado");
            }
        } catch (SQLException e) {
            logger.log(Level.SEVERE, "Error al borrar el producto en la base de datos", e);
        }
    ...
    public List<Flower<Integer>> findAll() {
        ...
        try (Statement stmt = connection.createStatement();
            ...
            while (rs.next()) {
                ...
                Flower<Integer> flower = new Flower<>(name, price, stock, color);
                flower.setId(id);
                flowers.add(flower);
            }
        } catch (SQLException e) {
    ...
    public int getTotalStockFlowers() {
        int totalStock = 0;
        try (Statement stmt = connection.createStatement();
             ResultSet rs = stmt.executeQuery("SELECT SUM(stock) FROM product WHERE type='FLOWER'")) {
            if (rs.next()) {
                totalStock = rs.getInt(1);
            }
        } catch (SQLException e) {
    ...
    public double getTotalValueFlowers() {
        double totalValue = 0;
        try (Statement stmt = connection.createStatement();
             ResultSet rs = stmt.executeQuery("SELECT SUM(stock * price) FROM product WHERE type='FLOWER'")) {
            if (rs.next()) {
                totalValue = rs.getDouble(1);
            }
        } catch (SQLException e) {
            logger.log(Level.SEVERE, "Error al leer el valor total de flores en la base de datos", e);
        }
        return totalValue;
    }
}

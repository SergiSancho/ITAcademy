...
public class MysqlTreeDao implements TreeDao {
    ...
    public boolean exists(Tree tree) throws SQLException {
        String sql = "SELECT COUNT(*) FROM product p JOIN tree t ON p.id_product=t.id_product WHERE p.name = ? AND t.height = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {

...
public class CreateDatabaseMysql {
    ...
    public static void createDatabase() {
        try (Connection connection = MysqlConnection.getInitialConnection();
             PreparedStatement createDBStatement = connection.prepareStatement(QueriesMysql.createDatabaseQuery())) {
            createDBStatement.executeUpdate();
            logger.info("Base de datos '" + MysqlConnection.DB_NAME + "' lista y operativa");
        } catch (SQLException e) {
            logger.log(Level.SEVERE, "Error creando base de datos '" + MysqlConnection.DB_NAME + "': " + e.getMessage(), e);
        }
    }
    ...
    public static boolean existsDatabase() {
        ...
        try (Connection connection = MysqlConnection.getInitialConnection();
            ...
            String checkDatabaseQuery = "SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = '" + MysqlConnection.DB_NAME + "'";
            try (ResultSet resultSet = statement.executeQuery(checkDatabaseQuery)) {
                if (resultSet.next()) {
                    exists = true;
                    logger.info("La base de datos '" + MysqlConnection.DB_NAME + "' ya existe");
                } else {
                    logger.info("La base de datos '" + MysqlConnection.DB_NAME + "' no existe");
                }
            }
        } catch (SQLException e) {
            logger.log(Level.SEVERE, "Error verificando la existencia de la base de datos '" + MysqlConnection.DB_NAME + "': " + e.getMessage(), e);
        }
        return exists;
    }
